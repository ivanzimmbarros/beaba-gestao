...
def _run_cag_consolidated_slice(cliente_id: int, ag_id: int) -> str | None:
    ...
    # Pesquisa unificada CAG (`cliente_search.py`): grelha rótulos / inputs+botão / sugestões — sem checkbox na área de busca.
    from pathlib import Path
...
def test_e2e_home_visual_shell_contract() -> None:
    """E2E leve: Início (cockpit) com Ilha Mãe + slot (paridade CAG/VND/COL/CAT)."""
