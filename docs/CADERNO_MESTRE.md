...
3. **Colaboradores:** nome, sexo, **data de nascimento** (idade mínima **18 anos**), **morada estruturada** (mesma regra que clientes), email, **número de contacto exclusivo** (11 dígitos, sem partilha entre colaboradores); **pelo menos um serviço** habilitado; por linha de serviço, **percentual de repasse** (0,01%–100,00%), **data de inserção da linha** (obrigatória, ISO; não é a data de cadastro do colaborador), possibilidade de **remover** linhas (mantendo ≥1) e **editar ficha** existente. Serviços ativos provêm de `servicos` (catálogo + seeds).
4. **Catálogo (E06 incremental):** naturezas **Sessão**, **Produto**, **Coworking** e **Pacote** (composição 1:N de sessões com quantidade e duração por linha ou herdada do catálogo; produto opcional; **repasse médio de referência** com sugestão automática ponderada pelas médias dos colaboradores habilitados em cada tipo de sessão e campo **editável**; valor de venda do pacote). **Evento** na Fase 3. **Descritivo** obrigatório; **Ativo/Inativo**; listagem na página. Valores em **centavos** onde aplicável. **Pacotes** não entram nas habilitações de colaboradores (`listar_servicos`).
