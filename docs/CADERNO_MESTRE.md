...
4. **Catálogo (E06):** naturezas **Sessão**, **Produto**, **Coworking**, **Pacote** (1:N sessões, produto opcional, repasse ref. sugerido+editável, valor venda) e **Evento** (data, local, observações, interno vs com convidado, preços criança/adulto e desconto filho adicional, participantes colaborador ou parceiro com repasse em % ou valor). **Descritivo** obrigatório; **Ativo/Inativo**; listagem na página. Valores em **centavos** onde aplicável. **Pacote** e **Evento** não entram em `listar_servicos` (habilitações).
